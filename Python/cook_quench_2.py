...
# Project 
pr_1 = Project("/nfshome/deshmukh/pyiron/projects/NASICON/project/hena/hena_2_2/minimization")           #! Path for postprocessing 
pr = Project("/nfshome/deshmukh/pyiron/projects/NASICON/project/hena/hena_2_2/glass")   
...
velocity 	    all create 1400 4928459 mom yes rot yes dist gaussian
fix		        mynvt all nvt temp 623 623 0.1
timestep	    0.001
...
fix		        mynvt all nvt temp 5000 5000 0.1
run		        100000
unfix           mynvt
#---------------------------------------------------------------------------------------------#
...
unfix           mynvt
#---------------------------------------- Quench NPT -----------------------------------------#
fix		        mynpt all npt temp 4000 300 0.1 iso 0.00 0.00 1.0
...
strct_no = [6, 25,  4, 41, 20]#, 28, 30, 35, 45, 34, 49] 

for k in strct_no:
    job_mini = pr_1['hena_2_1_struct_mini_%s'%k]  # Bulk Crystal
    # Get last structure    
    struct_nv = job_mini.get_structure(iteration_step=-1)    # Last frame from minimization
    struct_nvt = struct_nv.repeat([3,3,2])
    import random
    #ran = random.randrange(49284)
    for i, j in zip(t, rate):
        #ran = random.randrange(49284)
        job_name = 'hena_2_2_big_%s_rate_%s_k_ps'%(k,j)
        ...
        job_lmp.server.queue = "long_l2"    ### See the queues.yaml file for queue names. Standard queue on Lichtenberg is normal with a runtime limit of 24 hours.
