...
def compute_common_ids(path_line='dump_nvt_prod.out',step=1.0, dump_write=100, step_ovito=10, m=0, n=1000):
...
def smooth_msd(path_line='dump_nvt_prod.out', file_2='msd_na.txt', step=1.0, dump_write=100, step_ovito=10, m=0, n=1000, common_indices=[1,2,3,4,5,6]):
...
for i,j,k in zip(li,input_files,no):
    ...
    #common_na = compute_common_ids(path_line=j,step=2.0, dump_write=200, step_ovito=1, m=0, n=pipeline.source.num_frames)
    
    for v in range(0, pipeline.source.num_frames, 5):
        ...
        smooth_msd(path_line=j,file_2='msd_grain_2_%s_%s_%s_%s.txt'%(i,k,m,n),step=2.0, dump_write=200, step_ovito=1, m=m, n=n, common_indices=common_na)
        print('-----------------------------------------------Next calc----------------------------------------------------------')
        print('---------------------------------------------------------------------------------------------------------------------')
