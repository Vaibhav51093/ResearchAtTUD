        ...
        if solution:
            ...
            if balanced_coeff_B == 1 and balanced_coeff_C == 0 and balanced_coeff_A == 0 and balanced_coeff_D == 0 and balanced_coeff_E == 0:
                ...
                if d_energy < 0:
                    ...
                    data_1_4['DeltaE'].append(round(d_energy, 6))
                    possible_reaction_5.append(reaction)
                    reaction_energy_5.append(round(d_energy, 6))
                    print(f"{'%s'%interest} -> {balanced_coeff_A}*{i[0]} + {balanced_coeff_B}*{i[1]} + {balanced_coeff_C}*{i[2]} + {balanced_coeff_D}*{i[3]} + {balanced_coeff_E}*{i[4]}, deltaE = {round(d_energy, 6)} eV/atom")
                elif d_energy > 0:
                    data_2_4['Reactant'].append(interest)
                    data_2_4['Product'].append(reaction)
                    data_2_4['DeltaE'].append(round(d_energy, 6))
                    not_possible_reaction_5.append(reaction)
                    reaction_energy_not_5.append(round(d_energy, 6))
    else:
        pass
