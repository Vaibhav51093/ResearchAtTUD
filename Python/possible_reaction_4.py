       ...
       all(key in interest_2 for key in compo_D.keys()):
        ...
        if solution:
            ...
            if balanced_coeff_B == 1 and balanced_coeff_C == 0 and balanced_coeff_A == 0 and balanced_coeff_D == 0:
                ...
                d_energy = (balanced_coeff_A * en_A + balanced_coeff_B * en_B + balanced_coeff_C * en_C + balanced_coeff_D * en_D - desired_en)/form_int    # eV/atom
                reaction = f"{interest} -> {balanced_coeff_A}*{i[0]} + {balanced_coeff_B}*{i[1]} + {balanced_coeff_C}*{i[2]} + {balanced_coeff_D}*{i[3]}"
 
                if d_energy < 0:
                    data_1_2['Reactant'].append(interest)
                    data_1_2['Product'].append(reaction)
                    data_1_2['DeltaE'].append(round(d_energy, 3))
                    possible_reaction_4.append(reaction)
                    reaction_energy_4.append(round(d_energy, 3))
                    print(f"{'%s'%interest} -> {balanced_coeff_A}*{i[0]} + {balanced_coeff_B}*{i[1]} + {balanced_coeff_C}*{i[2]} + {balanced_coeff_D}*{i[3]}, deltaE = {round(d_energy, 3)} eV/atom")
                elif d_energy > 0:
                    data_2_2['Reactant'].append(interest)
                    data_2_2['Product'].append(reaction)
                    data_2_2['DeltaE'].append(round(d_energy, 3))
                    not_possible_reaction_4.append(reaction)
                    reaction_energy_not_4.append(round(d_energy, 3))
    else:
        pass
