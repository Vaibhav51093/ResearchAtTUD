...
#    print('All the NVE calculations (amorphous) are finished............Thank you for the patience')
    ...
    # Get the average size of the box (put this in the script)
    #xx_size = []
    #xy_size = []
    #xz_size = []
    #yy_size = []
    #yz_size = []
    #zz_size = []
    #for j in range(3000,5001,100):    # Last few frames 
    #    xx = job_mini_3.get_structure(iteration_step=j).cell[0,0]
    #    xy = job_mini_3.get_structure(iteration_step=j).cell[0,1]
    #    xz = job_mini_3.get_structure(iteration_step=j).cell[0,2]
    #    yy = job_mini_3.get_structure(iteration_step=j).cell[1,1]
    #    yz = job_mini_3.get_structure(iteration_step=j).cell[1,2]
    #    zz = job_mini_3.get_structure(iteration_step=j).cell[2,2]
    #    xx_size.append(xx)
    #    xy_size.append(xy)
    #    xz_size.append(xz)
    #    yy_size.append(yy)
    #    yz_size.append(yz)
    #    zz_size.append(zz)
    #xx_new = np.average(xx_new)
    #yy_new = np.average(yy_new)
    #zz_new = np.average(zz_new)
    #xy_new = np.average(xy_new)
    #xz_new = np.average(xz_new)
    #yz_new = np.average(yz_new)