...
def smooth_msd(path_line='dump_nvt_prod.out', file_2='msd_na.txt', step=1.0, dump_write=100, step_ovito=10, m=0, n=1000):
    ...
    # Replicate:
    pipeline.modifiers.append(ReplicateModifier(num_y = 2)) # to have another GB in the consideration 
...
#input_files = ["/nfshome/deshmukh/vaibhav/schulze_project/gb/gb_3/gb_13_230/relaxation_2/gb_13_120/01-heat-523/02-diffusion/structure.dump.*"]
    ...
    for f in range(0, pipeline.source.num_frames, 100):
        ...
        for v in range(0+f, pipeline.source.num_frames, 450):
            ...
            if n > pipeline.source.num_frames:
                ...
                #print(common_na)
                smooth_msd(path_line=j,file_2='msd_gb_2_%s_%s_%s_%s.txt'%(i,k,m,n),step=2.0, dump_write=200, step_ovito=1, m=m, n=n) #, common_indices=common_na)
                print('-----------------------------------------------Next calc----------------------------------------------------------')
                print('---------------------------------------------------------------------------------------------------------------------')
