...
for k in range(1, 2, 1):
    job_mini = pr['boilot_443K_2_05_amo_s_1_1_rate_%s_k_ps'%k]  # Bulk Crystal
    # Get last structure    
    struct_nvt = job_mini.get_structure(iteration_step=-1)    # Last frame from minimization
    import random
    #ran = random.randrange(49284)
    ran = 4928459
    job_name = 'boilot_443K_2_05_amo_s_1_eq_rate_%s_k_ps'%k
...
for k in range(2, 11, 1):
    job_mini = pr['boilot_443K_2_05_amo_s_1_rate_%s_k_ps'%k]  # Bulk Crystal
    # Get last structure    
    struct_nvt = job_mini.get_structure(iteration_step=-1)    # Last frame from minimization
    import random
    #ran = random.randrange(49284)
