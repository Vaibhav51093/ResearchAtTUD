...
for k in strct_no:
    job_mini = pr_1['hena_1_struct_mini_%s'%k]  # Bulk Crystal
    # Get last structure    
    ...
    for i, j in zip(t, rate):
        #ran = random.randrange(49284)
        job_name = 'hena_1_%s_rate_%s_k_ps'%(k,j)
        ...
        job_lmp.server.cores = 192             ### The number of cores you want for your job
        job_lmp.run()
        job_lmp.decompress()    
