...
#        self.results.timeseries = self.results.msds_by_particle.mean(axis=1)
    ...
    def _conclude_fft(self):
        ...
        print('We are using tidynamics modified by me')
        
        if 'xy' in self.msd_type:
            msd_xy = tidynamics.msd(self._position_array[:, :, [0,1]])
            self.results.msds_by_particle_xy = msd_xy
            self.results.timeseries_xy = np.mean(msd_xy, axis=1)
            cross_msd_xy = tidynamics.cross_displacement_msd(self._position_array[:, :, [0,1]])
            self.results.cross_msds_by_particle_xy = cross_msd_xy
            self.results.cross_timeseries_xy = np.mean(cross_msd_xy, axis=1)
        
        if 'xz' in self.msd_type:
            msd_xz = tidynamics.msd(self._position_array[:, :, [0,2]])
            self.results.msds_by_particle_xz = msd_xz
            self.results.timeseries_xz = np.mean(msd_xz, axis=1)
            cross_msd_xz = tidynamics.cross_displacement_msd(self._position_array[:, :, [0,2]])
            self.results.cross_msds_by_particle_xz = cross_msd_xz
            self.results.cross_timeseries_xz = np.mean(cross_msd_xz, axis=1)
        
        if 'yz' in self.msd_type:
            msd_yz = tidynamics.msd(self._position_array[:, :, [1,2]])
            self.results.msds_by_particle_yz = msd_yz
            self.results.timeseries_yz = np.mean(msd_yz, axis=1)
            cross_msd_yz = tidynamics.cross_displacement_msd(self._position_array[:, :, [1,2]])
            self.results.cross_msds_by_particle_yz = cross_msd_yz
            self.results.cross_timeseries_yz = np.mean(cross_msd_yz, axis=1)   
                
        if 'x' in self.msd_type:
            msd_x = tidynamics.msd(self._position_array[:, :, 0])
            self.results.msds_by_particle_x = msd_x
            self.results.timeseries_x = np.mean(msd_x, axis=1)

        if 'y' in self.msd_type:
            msd_y = tidynamics.msd(self._position_array[:, :, 1])
            self.results.msds_by_particle_y = msd_y
            self.results.timeseries_y = np.mean(msd_y, axis=1)

        if 'z' in self.msd_type:
            msd_z = tidynamics.msd(self._position_array[:, :, 2])
            self.results.msds_by_particle_z = msd_z
            self.results.timeseries_z = np.mean(msd_z, axis=1)
